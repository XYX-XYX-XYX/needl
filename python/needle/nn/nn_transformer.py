...
class MultiHeadAttention(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        z = self.matmul(q, k) / (q_dim ** 0.5)
        if self.causal:
            mask = self.create_causal_mask(queries_len, queries_len, z.device)
            mask = Tensor(mask, device=self.device, dtype=self.dtype, requires_grad=False)
...
class AttentionLayer(Module):
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        _q = self.q_projection(self.prenorm_q(q.reshape([q.shape[0] * q.shape[1], q.shape[2]])))
        _k = self.k_projection(self.prenorm_k(k.reshape([k.shape[0] * k.shape[1], k.shape[2]])))
        _v = self.v_projection(self.prenorm_v(v.reshape([v.shape[0] * v.shape[1], v.shape[2]])))
        mutihead_shape = [batch_size, queries_len, self.num_head, self.dim_head]

        _q = _q.reshape(mutihead_shape).transpose(tuple([1, 2]))
        _k = _k.reshape(mutihead_shape).transpose(tuple([1, 2]))
        _v = _v.reshape(mutihead_shape).transpose(tuple([1, 2]))
        ### END YOUR SOLUTION
        attention, probs = self.attn(_q, _k, _v)
        result = self.out_projection(attention.transpose(tuple([1, 2])).reshape([batch_size * queries_len, self.num_head * self.dim_head]))
        return result.reshape([batch_size, queries_len, self.out_features])
...
class TransformerLayer(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.laynorm1 = LayerNorm1d(q_features, device=device, dtype=dtype)
    ...
    def forward(
        ...
        ### BEGIN YOUR SOLUTION
        x = x + self.drop(self.attention(x))
        x = x.reshape([batch_size * seq_len, x_dim])
        x = x + self.drop(self.linear2(self.drop(self.activation(self.linear1(self.laynorm1(x))))))
        x = x.reshape([batch_size, seq_len, x_dim])
        return x
        ### END YOUR SOLUTION
...
class Transformer(Module):
    ...
    def forward(
        ...
        idx_np = np.arange(seq_len)
        idx = Tensor(idx_np, device=self.device, dtype=self.dtype, requires_grad=False).reshape([seq_len, 1])
        position_encode = self.embedding(idx)
        x = x + position_encode.reshape(tuple([1, seq_len, feature])).broadcast_to(x.shape)
